  ...
  methods: {
    ...
    checkRole(x) {
      if (store.getters.getRoles.includes('influencer')) {
        if (x['flag'] === true) {
          this.message = "Your account is flagged. Please contact support."
          this.log = true
          router.push('/login');
        }
        else {
        ...
        if (x['pending'] === false && x['approve'] === false) {
          this.message = "Your approval denied by admin."
          this.log = true
          router.push('/login');
        }
        else if (x['approve'] === false) {
